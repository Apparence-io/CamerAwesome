...
SaveConfig.photoAndVideo(
  ...
  videoOptions: VideoOptions(
    // TODODOC set enableAudio parameter here ?
  ...
  // TODODOC set exifPreferences here ?
)
```
Let's break it down:
...
Here is a `photoPathBuilder` example:
```dart
SaveConfig.photoAndVideo(
  photoPathBuilder: (sensors) async {
    // 1.
...
// TODODOC Either move these parameters or provide a better documentation
