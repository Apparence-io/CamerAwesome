...
```dart
state.when(
    onPictureMode: (pictureCameraState) => pictureCameraState.takePhoto(),
...
- **PreparingState** : camera is starting
- **PictureCameraState** : camera is ready to take a picture
...
As you can see, after the initial `PreparingState`, the new state is either `PictureCameraState` or `VideoCameraState`.
...
You can find some common widgets that you may want to use in the **Widgets** section.
...
|**Get** original AwesomeFileSaver|`state.awesomeFileSaver`||


#### PictureCameraState properties and methods

**Take a picture**
|Use case|Code|
|-|-|
|Take a picture|`state.takePhoto()`|

**Toggle to save (or not) the location when taking pictures**
|Use case|Code|
|-|-|
