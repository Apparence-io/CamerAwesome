  ...
  group('Check each element is present', () {
    patrolTest(
      ...
      ($) async {
        ...
        expect($(AwesomeAspectRatioButton), findsOneWidget);
        expect($(AwesomeFlashButton), findsOneWidget);
        expect($(AwesomeLocationButton).$(IconButton), findsNothing); // Not visible anymore
        expect($(AwesomeCameraSwitchButton), findsOneWidget);
    ...
    // TODO Make this work once PR on video lock mode is merged
    if (false)
      patrolTest(
        'Video mode elements',
        config: patrolConfig,
          ...
          await $.pumpWidgetAndSettle(
            DrivableCamera(
              ...
              saveConfig: SaveConfig.video(
                pathBuilder: () => tempPath('single_video_back.mp4'),
              ),
            ),
          );
